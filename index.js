...
function indekseGoreFenomen(fenomenler2, index) {
  let profile = fenomenler2[index]["profile"];
...
function fenomenEkle(fDizi, siraNo, profil, takipciS, gonderimS, platfrom) {
  let nDizi = [...fDizi];
  let yObje = {
    "number": siraNo,
    "profile": profil,
...
function enFenomenler(fDizi) { 
  let yDizi = [];
  for(let fenomen of fDizi){
    if(fenomen["followers"] > 100000000)
      yDizi.push(fenomen["profile"]);
  }
  return yDizi; 
}
...
function fenomenGonderimSayisi(fDizi, profil){ 
  for(let i = 0; i < fDizi.length; i++){ 
    if(fDizi[i].profile == profil){ 
